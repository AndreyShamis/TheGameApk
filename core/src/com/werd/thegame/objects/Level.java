package com.werd.thegame.objects;
...
public class Level {
    private static final String TAG = "TheGame-Level";
	...
	public Level(String level) {
        Gdx.app.log(TAG, "------------------- Loading Level " + level);
		Gdx.app.log(TAG, "------------------- XMLparse Level " + level);
		xml_parse = new XMLparse();
		Gdx.app.log(TAG, "------------------- XMLparseStars Level " + level);
		Array<Star> xml_stars = xml_parse.XMLparseStars();
		Gdx.app.log(TAG, "------------------- XMLparseNotes Level " + level);
		notes = xml_parse.XMLparseNotes(level);
		Gdx.app.log(TAG, "------------------- getPos Level " + level);
		starsPos = xml_parse.getPos(level);
		endNote = 3;
		delay = 0;
		this.win = false;
        Gdx.app.log(TAG, "------------------- Set Play Music " + level);
		setPlayMusic();
